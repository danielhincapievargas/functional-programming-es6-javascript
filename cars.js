...
const ferraris = () => {
  return cars.filter(({ brand }) => brand === 'Ferrari')
}
console.log(ferraris());

const nineties = () => {
  return cars.filter(({ model }) => model >= 1980 && model <= 1990)
}
...
bmwSum = () => {
  const bmwCars = cars.filter(({ brand }) => brand === 'BMW');
  const total = bmwCars.reduce((sum, { price }) => sum + price, 0);
  return total;
}
console.log(bmwSum());
